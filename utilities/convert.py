...
#
# https://www.ursa.fi/~lkangas/python/TM35FIN.py
#
# TM35FIN.py
#
...
def usage():
    print("Usage: convert inFilename outFilename")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print "Could not open file {}".format(sys.argv[2])
        exit(1)

    firstLine = inFile.readline().split(',')
    if firstLine[0].lower().strip() != 'x' or firstLine[1].lower().strip() != 'y':
       print "Expected csv format: x, y, ..."
       exit(1)
       
    outFile.write("LONGITUDE,LATITUDE,LABEL\n")
    for line in inFile:
        ...
        label = sss[2].strip() if len(sss) > 2 else "({},{})".format(lat,lon)
